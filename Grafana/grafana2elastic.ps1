...
function SendTo-Elasticsearch ($json, $elasticServer, $elasticIndex, $indexDate)
{
    try
    {
       Invoke-RestMethod "http://$elasticServer/$elasticIndex-$indexDate/message" -Method Post -Body $json -ContentType 'application/json'
    }
...
function Get-GrafanaMetrics ($grafanaServer)
{
    ...
    try
    {
        $a = Invoke-RestMethod -Uri "http://$grafanaServer/api/metrics"
        ...
        SendTo-Elasticsearch $json $elasticServer $elasticIndex $indexDate
    }
       catch [System.Exception]
       {
